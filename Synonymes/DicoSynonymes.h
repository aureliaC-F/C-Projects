   ...
   */
   ...
   private:
      ...
      */
      ...
      void _auxAjouterRadical(const std::string& motRadical, NoeudDicoSynonymes*& noeud);
      void _auxAjouterFlexion(const std::string& motRadical, const std::string& motFlexion, NoeudDicoSynonymes*& noeud);
      int  _auxsimilitude(const std::string& mot1, const std::string& mot2) const;
      void _auxSupprimerRadical(const std::string& motRadical, NoeudDicoSynonymes*& noeud);
      void _SupprimerSuccMinDroit(NoeudDicoSynonymes* noeud);
      void _auxDestructionNoeuds(NoeudDicoSynonymes* noeud);
      std::string _auxRechercherRadical(const std::string& mot, NoeudDicoSynonymes* noeud) const;


      //balancement
      void _balancer(NoeudDicoSynonymes*& noeud);
      int _hauteur(NoeudDicoSynonymes* noeud);
      //conditions
      bool _debalencementAGauche(NoeudDicoSynonymes* noeud);
      bool _sousArbrePencheADroite(NoeudDicoSynonymes* noeud);
      bool _debalencementADroite(NoeudDicoSynonymes* noeud);
      bool _sousArbrePencheAGauche(NoeudDicoSynonymes* noeud);
      //zig et zag
      void _zigZigGauche(NoeudDicoSynonymes*& noeud);
      void _zigZagGauche(NoeudDicoSynonymes*& noeud);
      void _zigZigDroit(NoeudDicoSynonymes*& noeud);
      void _zigZagDroit(NoeudDicoSynonymes*& noeud);



      NoeudDicoSynonymes* findNoeux(const std::string& motRadical) const
      {
          return _auxFindNoeux(motRadical, racine);
      }
      NoeudDicoSynonymes* _auxFindNoeux(const std::string& motRadical, NoeudDicoSynonymes* noeud) const
      {
          if (noeud == 0)
          {
              throw std::logic_error("le radical fourni n'existe pas");
          }
          else
          {
              if (motRadical < noeud->radical)
              {
                  return _auxFindNoeux(motRadical, noeud->gauche);
              }
              else if (motRadical > noeud->radical)
              {
                  return _auxFindNoeux(motRadical, noeud->droit);
              }
              else
              {
                  return noeud;
